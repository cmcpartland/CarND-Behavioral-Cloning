...
def generate_model():
	...
	model.fit_generator(training_generator, 
						samples_per_epoch = int(len(training_set)*3),
...
def generator(driving_log_lines, batch_size=32):
	...
	while True:
		...
		for offset in range(0, num_samples, batch_size):
			...
			images = np.empty((int(batch_size*3), 160, 320, 3), dtype=np.uint8)
			measurements = []
			...
			for i,line in enumerate(batch_lines):
				...
				current_path_center = 'data/data/IMG/' + filename_center
				current_path_left = 'data/data/IMG/' + filename_left
				current_path_right = 'data/data/IMG/' + filename_right
